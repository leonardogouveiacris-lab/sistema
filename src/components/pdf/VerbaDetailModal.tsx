...
const VerbaDetailModal: React.FC<VerbaDetailModalProps> = ({
  ...
  const createdAt = getSafeDate(lancamento.dataCriacao);
  const updatedAt = getSafeDate(lancamento.dataAtualizacao);
      ...
      <div className="absolute right-4 top-4 pointer-events-auto w-[480px] bg-white shadow-2xl flex flex-col border border-gray-200 rounded-xl max-h-[calc(100vh-2rem)] overflow-hidden">
            ...
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              {lancamento.paginaVinculada && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-green-100 text-green-700 border border-green-200 hover:bg-green-200 transition-colors"
                  title={`Ir para página ${lancamento.paginaVinculada}`}
                >
                  <FileText size={11} />
                  Página {lancamento.paginaVinculada}
                </button>
              )}
              {lancamento.situacao && (
                <span className={`inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full border ${situacaoColor}`}>
                  {lancamento.situacao}
                </span>
              )}
            </div>
            <h2 className="text-lg font-bold text-gray-900 leading-snug">
              {lancamento.decisaoVinculada || verba.tipoVerba}
            </h2>
          </div>
          <button
            onClick={onClose}
            className="flex-shrink-0 p-1.5 text-gray-400 hover:text-gray-700 hover:bg-green-100 rounded-lg transition-colors"
            title="Fechar (Esc)"
          >
            <X size={18} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          {lancamento.fundamentacao && (
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Fundamentação</p>
          ...
          {lancamento.comentariosCalculistas && (
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Comentários</p>
              <LancamentoRefRenderer
