...
const DocumentoDetailModal: React.FC<DocumentoDetailModalProps> = ({
  ...
  const createdAt = getSafeDate(documento.dataCriacao);
  const updatedAt = getSafeDate(documento.dataAtualizacao);
      ...
      >
        ...
        >
          <div className="absolute top-2 left-1/2 -translate-x-1/2 flex gap-0.5">
            <div className="w-8 h-1 rounded-full bg-orange-200" />
          </div>
            ...
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              {documento.paginaVinculada && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-orange-100 text-orange-700 border border-orange-200 hover:bg-orange-200 transition-colors"
                  title={`Ir para página ${documento.paginaVinculada}`}
                >
                  <FileText size={11} />
                  Página {documento.paginaVinculada}
                </button>
              )}
            </div>
          ...
          <button
            onClick={onClose}
            className="flex-shrink-0 p-1.5 text-gray-400 hover:text-gray-700 hover:bg-orange-100 rounded-lg transition-colors"
            title="Fechar (Esc)"
          >
            <X size={18} />
          </button>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex-shrink-0">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => { onEdit(documento.id); onClose(); }}
              ...
              <button
                onClick={() => { onDelete(documento.id); onClose(); }}
